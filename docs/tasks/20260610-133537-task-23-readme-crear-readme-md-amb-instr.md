...
- 2026-06-10 15:06: Revisió completa del projecte: corregits 4 problemes crítics (validació input, error handler, filtre fitxers, webhook secret), verificats Dockerfiles (build OK), verificades Clients pages (funcionament correcte), arreglat bug useApi amb useRef.
- 2026-06-10 15:06: 1. Correccions crítiques: input validation amb pick() a totes les rutes POST/PUT, errorHandler no exposa detalls en prod, filtre MIME a multer (CSV i imatges/PDF), verificació webhook amb secret, useApi hook arreglat (useRef), filtratge d'impagats arreglat. 2. Dockerfiles: corregit backend (npm ci sense --only=production per tenir prisma), afegit mkdir uploads, afegit .dockerignore a ambdós, ambdós Dockerfiles construeixen correctament. 3. Clients pages verificades: funcionen correctament, frontend compila net. → Implementat:sí Verificat:no Completat:no
